...
typedef struct ban_ma{
	unsigned char c[max];
    unsigned char khoa[max] ;
}text;

unsigned char khoa[max][max];
int dem;

void xor_acsii(char *a,char *b,unsigned char *keyA,unsigned char *keyB) {
    int len_a = strlen(a);
    int len_b = strlen(b);
    int c ;
    ...
    char space =' ';
    unsigned char ascii[c/2];
    unsigned char xorspace[c],xorngan[c],xordai[c];
   
    for(i = 0;i < c; i++){
	    xorchar[i]= (unsigned char)(a)[i] ^ (unsigned char)(b)[i]; // xor tung ki tu
	}
	...
	for(i=0;i<c/2;i++ ){
		if((ascii[i] >= 0x41 &&  ascii[i] <= 0x5A) || (ascii[i] >= 0x61 && ascii[i] <= 0x7A)) {
			ascii[i] = ascii[i]^space;
			
	}
		else {
			if(ascii[i] == 0x00 )  ascii[i] = 0x5F;    
		    else ascii[i] = 0x2A; 
		}
		printf("%c",ascii[i]);
	} 
    ...
    for(i=0;i<c;i++ ){
    	for(j=0; j< c/2;j++){
    		if((ascii[j] >= 0x41 &&  ascii[j] <= 0x5A) || (ascii[j] >= 0x61 && ascii[j] <= 0x7A)){
    			if(i % 2 == 0){
    				keyA[i] = 0x02 ^ (unsigned char)b[i];
    				keyB[i] = 0x02 ^ (unsigned char)a[i];		
				}else{
					keyA[i] = 0x00 ^ (unsigned char)b[i];
    				keyB[i] = 0x00 ^ (unsigned char)a[i];
				}
			}
		}
	}
...
* xor_khoa(char *a,unsigned char *b) {
    int len_a = strlen(a);
    int len_b = strlen(b);
...
int main() {
	...
	int j;
	int i;
	unsigned char key[11][11][500];
	 //xor tung ban ma voi nhau
    for(i = 0;i < 11;i++){
    	for(j=i+1;j< 11 ;j++){
    			printf("%d^%d: \n",i,j);
    		    xor_acsii(a[i].c,a[j].c,key[i][j],key[j][i]);
    		int k;
    		for(k = 0;k < strlen(key[i][j]);k++ ) printf("%x",key[i][j][k]);
    		printf("\n");
		}  
	}
	for(i = 0; i <10;i++){
		xor_khoa(a[10].c,key[10][i]);
	}
